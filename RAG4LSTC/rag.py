...
def main():
    ...
    loader = CSVLoader(file_path='../data/odp_categories.csv')
    docs = loader.load()

    print(f'The number of categories: {len(docs)}')
    print(docs[0])
    ...
    rag_chain = (
            {"candidate_categories": retriever | format_docs, "text": RunnablePassthrough()}
            | prompt
            | llm
            | StrOutputParser()
    )
    ...
    with open("../data/CID_Print.txt", "r", encoding='utf-8') as f:
        ...
        for line in lines:
            ...
            name_odpid_dict[id_odpid_name[2]] = id_odpid_name[1]
